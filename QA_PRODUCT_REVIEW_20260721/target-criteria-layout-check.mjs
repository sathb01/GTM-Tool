...
async function inspect(viewport) {
  ...
  await page.locator("#targetSearchOptionalSignals").evaluate((details) => { details.open = true; });
  ...
  const state = await page.evaluate(() => {
    ...
    const controls = [...document.querySelectorAll(".target-search-criteria-grid input, .target-search-criteria-grid textarea, .target-search-criteria-grid select")];
    ...
    return {
      ...
      compactHeights: controls.filter((control) => control.tagName === "INPUT" || control.tagName === "SELECT").map((control) => control.getBoundingClientRect().height),
      employeeRangeWidth: rect("#targetDiscoveryEmployeeRange").width,
...
try {
  ...
  check("Compact answer boxes use one consistent height", sameHeight(desktop.compactHeights), JSON.stringify(desktop.compactHeights));
  check("Employee range uses one full-width dropdown", desktop.employeeRangeWidth >= desktop.singleWidth * 1.9, JSON.stringify({ employeeRange: desktop.employeeRangeWidth, single: desktop.singleWidth }));
