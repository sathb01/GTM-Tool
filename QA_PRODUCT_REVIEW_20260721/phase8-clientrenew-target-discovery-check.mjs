...
check("Criteria editing is optional before the primary Find action", /id="editTargetSearchCriteria"/.test(targetSource) && /id="targetSearchCriteriaEditor"/.test(targetSource) && /id="buildTargetSearchPack">Find target companies/.test(targetSource));
...
check("No paid browser research is invoked", /does not run paid research automatically/.test(targetSource) && !/fetch\([^)]*api\/research/.test(targetSource));
check("Controlled AI prompt is copy-only", /id="copyTargetDiscoveryPrompt">Copy complete research request/.test(targetSource) && /copyTextToClipboard\(prompt\)/.test(targetSource));
check("Candidate import requires sourced structured fields", /Each company must include a public source and separate observed evidence from inference/.test(targetSource));
...
check("Candidate review clearly labels hypotheses", /Inference — verify before use/.test(targetSource));
...
check("Next-batch feedback captures all approved patterns", ["preferredSignal", "industry", "geography", "size", "technology", "referral", "exclusion"].every((field) => targetSource.includes(`data-discovery-feedback="${field}"`)));
check("Accepted companies require manual HubSpot verification", /accepted for manual verification/i.test(targetSource) && /No CRM connector is used/.test(targetSource));
